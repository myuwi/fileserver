...
type Props = {
    ...
    setViewerMedia: (media: ViewerMedia) => void;
}
...
export const ItemList = ({
    ...
    return (
        <div className={`flex flex-col flex-auto relative ${settings.viewMode === 'grid' ? ' overflow-y-auto' : 'overflow-hidden'}`}
            ...
            {fetching ? (
                ...
                files.length ? (
                    settings.viewMode === 'grid' ? (
                        <div
                            className={`grid justify-between items-start ${isMobile ? `${uiSizeClasses[settings.uiSize]} gap-y-2 gap-x-0 mt-0 mb-4 mx-4` : 'grid-cols-128px my-4 mx-8 gap-1'}`}
                            ref={itemlistRef}
                        >
