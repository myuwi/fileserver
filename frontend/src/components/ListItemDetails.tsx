...
export const ListItemDetails = React.forwardRef(
    (
        ...
        const columns = [
            ...
            {
                id: 'TYPE',
                content: item.directory ? 'Folder' : `${extension?.toUpperCase()} File`,
            },
            {
                id: 'SIZE',
        ...
        return (
            <>
                <div
                    className={`outline-none p-2 transition-colors duration-50 ease-in-out ${
                        selected ? 'bg-primary-500 bg-opacity-20' : 'hover:bg-secondary-500 hover:bg-opacity-5'
                    } select-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-opacity-100 flex flex-row flex-1`}
                    ref={ref}
                    tabIndex={0}
                    {...eventHandlers}
                    {...rest}
                ...
                >
                    <div className="pl-2 pr-4 text-primary-500">
                        {!item.directory && item.hasThumb ? (
                            ...
                            <Icon icon={item.directory ? mdiFolder : fileTypeIcon} size="18" />
                        )}
                    </div>
                    {columns.map((e, i) => {
                        return (
